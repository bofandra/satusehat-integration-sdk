...
> **Status:** draft/reference implementation siap dikembangkan di GitHub. Repo ini bukan rilis resmi Kementerian Kesehatan sampai ditetapkan melalui proses governance/release resmi.
...
## Catatan namespace package

Nama package/namespace di repository ini (`@satusehat/...`, `id.kemkes.satusehat`, `Satusehat.IntegrationSdk`, dan lainnya) adalah usulan untuk reference implementation. Sebelum publikasi resmi, pastikan ownership namespace/package registry dan branding telah disetujui.
...
event_id = sdk.enqueue("POST", "Encounter", payload=encounter)
...
Untuk production disarankan worker terpisah dari web request SIMRS agar restart web tidak menghambat drain queue. Pada backend SQLite default, gunakan **satu worker aktif per file queue**; kebutuhan multi-worker/multi-instance sebaiknya memakai shared queue adapter.
