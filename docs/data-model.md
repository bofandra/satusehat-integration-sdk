...
`idempotency_key` is an optional facility/vendor business key. When present, the queue enforces uniqueness per organization and returns the original `event_id` for repeated enqueue attempts. Recommended values are stable source identifiers such as `encounter:{local_visit_id}:create` or `observation:{local_order_id}:{loinc_code}:final`.
