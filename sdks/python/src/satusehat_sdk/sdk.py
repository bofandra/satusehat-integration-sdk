...
class SatusehatSdk:
    def __init__(self, config: SatusehatConfig):
        ...
        self.queue = SQLiteQueue(config.queue_path, config.processing_timeout_seconds)
        self.rate = _RateLimiter(config.rate_limit_rpm)

    def close(self):
        self.queue.close()

    def enqueue(self, operation: str, resource_type: str, resource_id: str | None = None, payload=None, idempotency_key: str | None = None) -> str:
        return self.queue.enqueue(self.config.organization_id, operation, resource_type, resource_id, payload, idempotency_key)

    def queue_stats(self) -> dict[str, int]:
        return self.queue.stats()

    def dead_letters(self, limit: int = 50) -> list[dict]:
        return self.queue.dead_letters(limit)

    def requeue(self, event_id: str) -> bool:
        return self.queue.requeue(event_id)
